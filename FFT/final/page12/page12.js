...
function createPage12(target, data, width, height)
{
	...
	var rightBody = mainBody.append("td").append("div").attr("style","width:100%; height:"+height+"px;border:5px solid #abc; position: relative;border-collapse: separate;");
...
/*************************************** right chart *************************************************/
	rightBody.append("div").attr("style", "background-color:#abc;").text("hello world");
	...
	for(var key in sortable)
	{
		 rightBody.append("div")
			...
			.attr("style", "height:"+height/data.length/4*3+"px;width:"+ (Math.abs(sortable[key].times)*leftBodySizeScale)+"px;") //Math.abs 取絕對值
	...
	/*右圖下方的下拉式選單*/
	var rightBodyDiv=rightBody.append("div").attr("style", "background-color:#abc; position:absolute; bottom:0; width:100%");
	rightBodyDiv=rightBodyDiv.text("Frequency").append("select").attr("class","selectpicker").attr("style", "width:50%");
	rightBodyDiv.append("option").text("first");
...
/*********************************** left chart ********************************************/
	...
	var svg = leftBody.append("svg")
		...
		.attr("style", "background-color:#def;")
	  .append("g")
